...
There are three types of missing data mechanisms: MCAR(missing completely at random), MAR(missing at random), and MNAR(missing not at random). Many imputation methods, including multiple imputation, assume MCAR or MAR. However, MCAR is the most difficult assumption to fulfill, since it asks that the missingness not to depend on either the observed or missing data. MAR is a little looser, but still expects the missingness to be independent from the missing values. To assess the adequacy of the assumptions, an important step is to review the data generation
process. Besides that we can follow a process of elimination: the missing pattern is believed MCAR unless strong opponent evidence appears. If MCAR is violated, then we believe the pattern is MAR, unless there is evidence against.
