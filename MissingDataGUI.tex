...
(air.temp and humidity) in the data \code{tao} follows MCAR or MAR.
...
The \pkg{MissingDataGUI}  package makes it possible to explore
...
structure, as can be done in \proglang{GGobi} and \proglang{MANET}.


\section*{Software}

\pkg{MissingDataGUI} is written in \proglang{R} 3.1.1 \citep{r} and
based on the package \pkg{gWidgets} \citep{gwidgets} with the
toolkit \proglang{RGtk2}. On different platforms (Windows, Linux,
...
are generated using \pkg{ggplot2} \citep{ggplot2}. The scatterplot
matrix and parallel coordinates plot are produced by package
\pkg{GGally} \citep{ggally}.
