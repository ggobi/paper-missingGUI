...
On the other hand, for some model-based imputation methods, it is important to check the assumptions, like missing completely at random (MCAR) or missing at random (MAR), but they are not easy to verify. \citet{little1988test} provided the tests for MCAR under the normality assumption, and \citet{jaeger2006testing} proposed an MAR test under the distributional assumption. Both tests employed the likelihood inferences, and gave the caution that the tests for MAR and MCAR are sensitive to model misspecification \citep{little1988test}, and MAR is non-testable without further assumptions \citep{jaeger2006testing}. Currently we do not find any \proglang{R} packages that can directly check the missingness assumptions, so a visual check will have some advantages - it may not confirm any randomness assumption, but it helps to deny the MCAR, or suggest the conditioning variables for MAR.
...
A number of imputation methods are available in the package. The purpose of these is not only to examine the dependence between missings or non-missings, but also to produce a complete data set for later analysis. A few criteria were considered to improve user experience: (1) easy to understand and implement; (2) computing complexity is medium or low; (3) adaptability to different situations, i.e., not need strong model assumptions. However, not all the methods that follows the criteria are applied in our package, because (1) there are plenty of methods as well as their variations, it is not practical to list them all; and (2) users may try their own method and import the result to missing data GUI.
...
\end{tabular}
\par\end{centering}
\caption{Imputation methods included in missing data GUI. Strictly speaking, 
...
This method requires at least one case in the dataset to be complete, and no character variables (ordinal variables are treated as integers). If there are less than $k$ complete cases, then all of them are used to generate the mean or a random value. If none of the cases are complete, then the mean or a random value of the entire data will be returned. $k$ is editable, and by default $k=5$.
...
Parallel coordinates plot is originated by \citet{inselberg1985plane} to visualize high-dimensional data. In missing data GUI, all the selected variables are paralleled in two orders. One is the original order in the data; the other sorts the coordinates from the best separator to the worst by F statistic from ANOVA. A perfect separator could distinguish the imputes of the colored-by variable(s) completely from the observation. Hence under the ``Below 10\%'' setting, the 
...
selected incomplete variables should have higher power on separation; and by ``overall mean'' or ``Random Value'', the selected incomplete variables would have lower power. On the other hand, the selected variables without missing values should be marked if they have a higher rank on their separation capacity, because the observations are likely to be incomplete in a special area of these variables. The plots are produced by function \code{ggparcoord} from \pkg{GGally}.
...
\item Save the plots. Plots can be saved tp png files by ``Save plot'' button. The imputation method and plot type will be auto-completed in the file name.
\end{itemize}
...
For each of the three incomplete variables, whether the MCAR or MAR assumption holds, should be considered before using the multiple imputation methods. However, as discussed in the first chapter, those assumptions are hard to verify without any further assumptions.

Compared to MAR, MCAR is a stronger condition and can be rejected when the missingness depends on any observed data (or missing data). Assume that the incomplete variable is not independent from other variables, then any special missing patterns on the other variables could be the evidences to reject MCAR.

Though many plot types, like the scatterplot or histogram, are helpful to reveal the missing pattern, they are not convenient to display many variables simultaneously. Parallel coordinates plot, inversely, can give an overview on a relatively large quantity of variables. If the two colors have quite different distributions in some variables, then it can be treat as an evidence of not being MCAR.

\begin{center}
\begin{figure}[h]
\begin{centering}
\begin{tabular}{cc}
{\tiny{(a) sea.surface.temp}} & {\tiny{(b) air.temp}}\tabularnewline
\includegraphics[width=.48\textwidth]{graph/fig12-1-sea-temp-2} &
\includegraphics[width=.48\textwidth]{graph/fig12-2-air-temp-2}\tabularnewline
{\tiny{(c) humidity}} & \tabularnewline
\includegraphics[width=.48\textwidth]{graph/fig12-3-humidity-2} & \tabularnewline
\end{tabular}
\par\end{centering}
\caption{Parallel coordinates plots colored by whether missing on sea.surface.temp(top left), air.temp(top right), or humidity(bottom left). The variables are sorted by the F statistic of ANOVA, i.e., if the difference between the missing data and the observed data is significant. Any missing values in the present variables are imputed by ``Below 10\%'' method. Obviously the missingness on air.temp and humidity depends on other variables, so the MCAR assumption on these two variables are violated.}
\label{fig:pcpCheck}
\end{figure}
\par\end{center}

Figure~\ref{fig:pcpCheck} gives the parallel coordinates plot for each of the three incomplete variables, colored by the missingness of the current incomplete variable. For example, the top left is based on sea.surface.temp. Then the other 7 variables are given in the plot, and the yellow lines are the cases that are missing only on sea.surface.temp. The missing values on air.temp or humidity are imputed by ``Below 10\%'' method. Only 3 out of 736 observations are missed on sea.surface.temp, so we can hardly claim any missing structure. But most of the missings on air.temp occurred in one year and one location, with higher sea.surface.temp, higher uwind, and lower humidity. Most of the missings on humidity happened in the same location as the missings on air.temp but another year, with lower sea.surface.temp and lower air.temp. Those are very strong evidences of violating MCAR.

\begin{center}
\begin{figure}[h]
\begin{centering}
\begin{tabular}{cc}
{\tiny{air.temp}} & {\tiny{humudity}}\tabularnewline
...
MAR states that the probability of missing depends on the observed values, but not on the missing values. Without other assumptions, we are not able to check it from the plots, since the real values of the missings are not available. Even when we see the obvious difference between missings and non-missings in Figure~\ref{fig:uvwind}, i.e., the missings of air.temp higher in uwind and more scattered in vwind, we can declare nothing on MAR, because given uwind and vwind (and other dependent variables), the missingness of air.temp may still be independent from its own missing values.

Nevertheless, the plots could suggest the possible conditioning variables, not only categorical but also numeric ones. From Figure~\ref{fig:uvwind} we see that the missingness of air.temp is conditioned on uwind and vwind, but humidity is not. For those suggested categorical conditioning variables, we can check them in the ``condition on'' widget, to get a better imputation.
